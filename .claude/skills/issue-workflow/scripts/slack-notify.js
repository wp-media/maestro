#!/usr/bin/env node
// PostToolUse hook — forwards maestro pipeline GitHub comments to Slack
'use strict';
...
const CHANNEL_ID  = process.env.MAESTRO_SLACK_CHANNEL || 'C0B680PH44T'; // #maestro-issue-workflow (fallback to wp-rocket for now)
const THREADS_DIR = path.join(process.cwd(), '.TemporaryItems', 'Issues', 'maestro', 'slack-threads');
...
async function run(raw) {
  ...
  const ghUrl    = isIssue
    ? `https://github.com/wp-media/maestro/issues/${refNum}`
    : `https://github.com/wp-media/maestro/pull/${refNum}`;
...
async function handlePrReady(command, output) {
  ...
  if (!prNum) return;
  const prUrl = `https://github.com/wp-media/maestro/pull/${prNum}`;
  ...
  await postToSlack({
    ...
    blocks: [
      ...
      { type: 'context', elements: [{ type: 'mrkdwn', text: '`wp-media/maestro`' }] }
    ]
  });
}
...
function buildStageCard(icon, label, body, num, ghUrl, commentUrl, isIssue, detailFn) {
  ...
  blocks.push({
    type: 'context',
    elements: [{ type: 'mrkdwn', text: `\`wp-media/maestro\`  ·  <${ghUrl}|${refType} #${num}>  ·  <${commentUrl}|Open in GitHub>` }]
  });
...
function buildGenericCard(body, num, ghUrl, commentUrl, isIssue) {
  ...
  return {
    ...
    blocks: [
      {
        type: 'section',
        text: { type: 'mrkdwn', text: `🤖  *${title}*\n<${ghUrl}|${refType} #${num}> · \`wp-media/maestro\`` },
        accessory: { type: 'button', text: { type: 'plain_text', text: 'View Comment' }, url: commentUrl }
      },
