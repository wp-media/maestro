...
const processEvent = db.transaction((hookType, data) => {
  ...
  switch (hookType) {
    ...
    case "UserPromptSubmit": {
      ...
      // Capture the first user prompt as a human-readable label. Hook payloads
      // carry the prompt under different keys depending on the emitter
      // (hook.mjs uses prompt_preview; CAM's hook-handler.js may use message
      // or prompt). Use the first non-empty one.
      const promptText = data.message || data.prompt || data.prompt_preview || null;
      if (promptText && typeof promptText === "string") {
        const trimmed = promptText.trim();
        if (trimmed) {
          // Agent task: first 80 chars — only set when still null so we keep
          // the *first* prompt of the session rather than overwriting it on
          // every subsequent turn.
          const taskLabel = trimmed.slice(0, 80);
          if (mainAgent && (mainAgent.task === null || mainAgent.task === "")) {
            db.prepare(
              `UPDATE agents SET task = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
               WHERE session_id = ? AND type = 'main' AND (task IS NULL OR task = '')`
            ).run(taskLabel, sessionId);
          }

          // Session name: first 60 chars — replaces the auto-generated
          // "{cwd_basename} - {sessionId}" label with the readable prompt, but
          // only while the name still looks auto-generated (never seeded from a
          // prompt before). We detect that via a NULL/blank name or the
          // "Session {id8}" default written by ensureSession.
          const nameLabel = trimmed.slice(0, 60);
          const curSession = stmts.getSession.get(sessionId);
          const looksAutoGenerated =
            !curSession?.name ||
            curSession.name === `Session ${sessionId.slice(0, 8)}` ||
            curSession.name === `Session ${sessionId.slice(0, 8)}`.trim();
          if (curSession && looksAutoGenerated) {
            stmts.updateSession.run(nameLabel, null, null, null, sessionId);
            broadcast("session_updated", stmts.getSession.get(sessionId));
          }
        }
      }
      ...
      // Re-read main agent so the broadcast above reflects the freshly-set task.
      if (mainAgentId) {
        broadcast("agent_updated", stmts.getAgent.get(mainAgentId));
      }
      break;
