...
function StepPill({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-indigo-50 dark:bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/20 whitespace-nowrap">
      {label}
    </span>
  );
}
...
function StepFlow({ steps }: { steps: string[] }) {
  ...
  return (
    <div className="flex items-center flex-wrap gap-1 min-w-0">
      ...
      {overflow > 0 && (
        <span className="inline-flex items-center px-2 py-1 rounded-lg text-xs font-medium bg-gray-700/50 text-gray-600 dark:text-gray-400 border border-gray-600/20 whitespace-nowrap">
...
function PatternFrequency({ count, percentage }: { count: number; percentage: number }) {
  ...
  return (
    <div className="flex-shrink-0 text-right">
      <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">{count.toLocaleString()}</p>
...
function PatternItem({ pattern, rank, isSelected, onClick }: PatternItemProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Rank / icon */}
        <div className="flex-shrink-0 w-7 h-7 rounded-md bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 flex items-center justify-center">
          {rank <= 3 ? (
            <span className="text-xs font-bold text-indigo-700 dark:text-indigo-400">{rank}</span>
          ) : (
            <Icon className="w-3.5 h-3.5 text-indigo-700 dark:text-indigo-400" />
          )}
        </div>
...
function PatternDetail({ pattern }: { pattern: WorkflowPattern }) {
  ...
  return (
    <div className="border-t border-indigo-200 dark:border-indigo-500/20 bg-surface-1/40 px-4 py-3.5 space-y-3.5">
      {/* Full step sequence (no truncation) */}
      <div>
        <p className="text-[10px] font-semibold text-gray-700 dark:text-gray-500 uppercase tracking-wider mb-2">
          {t("patterns.detail.stepsHeading")}
        </p>
        <div className="flex items-center flex-wrap gap-1.5">
          {pattern.steps.map((step, i) => (
            <span key={i} className="flex items-center gap-1.5">
              <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-indigo-50 dark:bg-indigo-500/15 text-indigo-800 dark:text-indigo-200 border border-indigo-500/25">
                <span className="text-indigo-700 dark:text-indigo-400/70 mr-1.5 text-[10px] font-bold">{i + 1}</span>
                {step}
              </span>
      ...
      <div>
        <p className="text-[10px] font-semibold text-gray-700 dark:text-gray-500 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
      ...
      <div className="bg-indigo-500/5 border border-indigo-500/15 rounded-md px-3 py-2.5">
        <p className="text-[10px] font-semibold text-indigo-700 dark:text-indigo-300 uppercase tracking-wider mb-1 flex items-center gap-1.5">
...
function DetailStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-surface-2 border border-border rounded-md px-2.5 py-2">
      ...
      <p className="text-[10px] text-gray-700 dark:text-gray-500 uppercase tracking-wider mt-0.5 truncate">{label}</p>
    </div>
  );
}
...
function EmptyPatterns() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{t("patterns.noData")}</p>
