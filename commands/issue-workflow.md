...
Repository: `{REPO}` (from `.claude/maestro.json`)
...
2. **Fetch the issue** — run `bash .claude/skills/issue-workflow/scripts/issue-sync.sh <N>` (or use the MCP equivalent). Read the resulting file at `{TEMP_ROOT}/issues/<N>/issue.md`.
