...
if (runParallel) {
  ...
  if (!skipLeadReview) {
    thunks.push(
      () => agent(reviewPrompt, { label: 'lead-review', phase: 'Quality Gates', schema: REVIEW_SCHEMA, agentType: 'maestro:lead-reviewer' })
    )
  }
  if (!skipQa) {
    thunks.push(
      () => agent(qaPrompt, { label: 'qa', phase: 'Quality Gates', schema: QA_SCHEMA, agentType: 'maestro:qa-engineer' })
    )
  }
  ...
  if (!skipLeadReview) {
    ...
    reviewResult = await agent(reviewPrompt, { label: 'lead-review', phase: 'Quality Gates', schema: REVIEW_SCHEMA, agentType: 'maestro:lead-reviewer' })
  }

  if (!skipQa) {
    log('Running QA...')
    qaResult = await agent(qaPrompt, { label: 'qa', phase: 'Quality Gates', schema: QA_SCHEMA, agentType: 'maestro:qa-engineer' })
  }
}
