...
**Severity tag:** Begin every inline comment body with a severity label mapping: CRITICAL/HIGH → `**High**`, MEDIUM → `**Medium**`, LOW → `**Low**`. Example opening: `**High** — Missing nonce check before processing user input.`
