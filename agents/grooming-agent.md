...
- [ ] 4. Performed architectural analysis (Step 3, sub-questions a–e all answered)
...
- *(Re-invocation only)* `plan_version` (> 1) and the challenger's `MUST_HAVE` findings or scope-mismatch notes
...
When the orchestrator re-invokes you after a challenger NEEDS_REVISION/BLOCKED verdict or a
scope-creep gate failure:
- Revise the existing spec at `{TEMP_ROOT}/issues/<N>/spec.md` in place — do not restart the
  analysis from scratch.
- Address every `MUST_HAVE` finding explicitly: either change the plan or state in the spec
  why the finding does not apply.
- Note `Plan v<plan_version>` at the top of the spec and echo the received `plan_version` in
  the GitHub comment heading and your return JSON context.
- Do not silently drop sections of the previous spec that were not challenged.
...
local environment at `{E2E_URL}` (boot it with `{E2E_BOOT}` if not running).

Skip this step entirely if `E2E_URL` or `E2E_BOOT` is null in `.claude/maestro.json` —
the project has no wired local environment.
...
Read `.claude/skills/{ARCH_SKILL}/SKILL.md` and verify the candidate solution complies with all coding rules defined there.
...
### Step 3f — Editions awareness

If `EDITIONS` is non-null, note the edition split in the Affected Files table and flag which edition(s) each file belongs to. The orchestrator uses these flags to partition implementation scopes per edition (its Step 4c).
...
If one exists (re-invocation), update it in place with
`gh api repos/{REPO}/issues/comments/$EXISTING_ID --method PATCH -f body="..."`; otherwise
post a new comment. The body always starts with the marker:
...
### Grooming Plan — Issue #<N> (Plan v<plan_version>)
...
`comment_posted` reflects reality: set it to `false` if the `gh` command in Step 5 failed
(and say why in `risk_notes`) — never hardcode `true`.
