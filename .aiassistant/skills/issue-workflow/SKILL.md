...
Read `.aiassistant/config/repo-map.json` to resolve project-specific values:
...
Repository: `{REPO}` (from `.aiassistant/config/repo-map.json`)
...
2. **Fetch the issue** — run `bash .aiassistant/skills/issue-workflow/scripts/issue-sync.sh <N>` (or use the MCP equivalent). Read the resulting file at `{TEMP_ROOT}/issues/<N>.md`.
