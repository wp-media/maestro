...
## Scope
Templates and PHP that use `echo`, `print`, `printf` across the project's template
...
```php
esc_html__( 'Clear Cache', '{ai.text_domain}' )
esc_attr__( 'Plugin Settings', '{ai.text_domain}' )
```
...
```php
printf(
    esc_html__( 'Cache cleared for %s.', 'text-domain' ),
    esc_html( $url )
);
```
...
echo wp_kses(
    sprintf(
        __( 'See <a href="%s">documentation</a>.', 'text-domain' ),
        esc_url( $url )
    ),
